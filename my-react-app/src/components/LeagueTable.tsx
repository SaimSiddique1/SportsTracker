  ...
  return (
    <div className="space-y-6">
      ...
      <div
        ...
        className="flex flex-wrap gap-2 border-b-2 border-slate-200 pb-4 dark:border-zinc-700"
      >
        {leagues.map((league) => {
          ...
          return (
            <button
              ...
              className={`border-2 px-3 py-1 text-[10px] font-black tracking-tighter transition-all
                focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-yellow-400
                ${active
                  ? "border-black bg-yellow-400 text-black dark:border-yellow-400"
                  : "border-transparent bg-transparent text-slate-600 opacity-50 hover:opacity-100 dark:text-zinc-400 dark:hover:text-zinc-100"
                }`}
            >
              {league.label}
            </button>
      ...
      <div role="region" aria-label={`${currLeague.name} standings`} aria-live="polite" aria-busy={loading}>
        {/* Column headers */}
        <div
          className="grid grid-cols-6 px-2 text-[10px] font-black text-slate-400 dark:text-zinc-500"
          aria-hidden="true"
        >
        ...
        {loading ? (
          <p className="px-2 py-4 text-sm font-semibold text-slate-500 dark:text-zinc-400">
            Loading standings…
          </p>
        ) : error ? (
          <p role="alert" className="px-2 py-4 text-sm font-semibold text-red-600 dark:text-red-400">
            {error}
          </p>
        ) : (
          /* Accessible table for screen readers, styled to match the original grid look */
            ...
            <tbody>
              {teams.map((team) => (
                <tr
                  ...
                  className="
                    grid grid-cols-6 items-center border-b border-slate-100 px-2 py-3
